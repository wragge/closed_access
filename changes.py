...
def find_date_changes(old=MONGOLAB_URL_2018, new=MONGOLAB_URL_2017):
    '''
    Files that were closed in both harvests but had different decision dates.
    Probably becuase they were WPA in old harvest.
    '''
    ...
    for item_old in old_db.items.find().batch_size(50):
        ...
        if item_new:
            if item_old['access_decision']['start_date']['date'] != item_new['access_decision']['start_date']['date']:
                print barcode
                date_changed += 1
        else:
            if item_old['access_decision']['start_date']['date'] >= datetime(2017, 1, 1, 0, 0, 0):
                changed += 1
                print '{}: {}, {}'.format(item_old['access_decision']['start_date']['date'], item_old['series'], item_old['_id'])
    print date_changed
    print changed
...
def get_reason_changes(db, exclude_missing=True, exclude_wpa=False):
    query = {}
    if exclude_missing:
        query["update.access_status"] = {"$ne": "Missing"}
    if exclude_wpa:
        query["reasons"] = {"$nin": ["Withheld pending adv"]}
    pipeline = [
            {"$match": query},
            {"$unwind": "$reasons"},
...
def get_reasons(db, decision_year=None, exclude_wpa=False):
    '''
...
def plot_reasons_changes(dbase={'year': '2017', 'db': MONGOLAB_URL_2018}, exclude_wpa=False):
    client = MongoClient(dbase['db'])
    db = client.get_default_database()
    if exclude_wpa:
        reasons = get_reasons(db, 2017, exclude_wpa=True)
        changes = get_reason_changes(db, exclude_wpa=True)
        filename = 'totals-by-reason-changes-excluding-wpa{}'.format(dbase['year'])
        title = '<b>Closed files</b><br>Changes in {} (excluding \'Withheld pending adv\')'.format(dbase['year'])
    else:
        reasons = get_reasons(db, 2017)
        changes = get_reason_changes(db)
        filename = 'totals-by-reason-changes-{}'.format(dbase['year'])
...
def plot_reasons_changes_nowpa(dbase={'year': '2017', 'db': MONGOLAB_URL_2018}):
    client = MongoClient(dbase['db'])
    db = client.get_default_database()
    reasons = get_reasons(db, 2017, exclude_wpa=True)
    changes = get_reason_changes(db, exclude_wpa=True)
    ...
    fig = go.Figure(data=data, layout=layout)
    plot_url = py.plot(fig, filename='totals-by-reason-changes-excluding-wpa{}'.format(dbase['year']))
